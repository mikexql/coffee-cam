...
extern bool useAdaptiveLighting;
...
static esp_err_t action_handler(httpd_req_t *req)
{
  ...
  if (buf_len > 1)
  {
    ...
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK)
    {
      ...
      httpd_query_key_value(buf, "adaptive", adaptive_str, sizeof(adaptive_str));
    }
    useAdaptiveLighting = (adaptive_str[0] != '0');
    free(buf);
  }
